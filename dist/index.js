...
function resolveApiKey(provider, readEnv) {
    const keyMap = {
...
export function shouldOmitTemperatureForApi(api) {
...
function resolveProviderApiFromRuntimeConfig(runtimeConfig, provider) {
    if (!isRecord(runtimeConfig)) {
        return undefined;
    }
    const providers = runtimeConfig.models
        ?.providers;
...
function parseAuthProfileStore(raw) {
    try {
        ...
        const order = rawOrder
            ? Object.entries(rawOrder).reduce((acc, [provider, value]) => {
...
function mergeAuthProfileStores(stores) {
    if (stores.length === 0) {
        return undefined;
    }
...
function resolveAuthProfileCandidates(params) {
    ...
    if (isRecord(params.runtimeConfig)) {
        ...
        if (isRecord(auth)) {
            const order = findProviderConfigValue(isRecord(auth.order) ? auth.order : undefined, params.provider);
...
async function resolveApiKeyFromAuthProfiles(params) {
    ...
    const persistPath = params.agentDir?.trim() ? join(params.agentDir.trim(), "auth-profiles.json") : storesWithPaths[0]?.path;
    for (const profileId of candidates) {
        ...
        const expires = credential.expires;
        const isExpired = typeof expires === "number" && Number.isFinite(expires) && expires > 0 && Date.now() >= expires;
        if (!isExpired && access) {
...
function createLcmDependencies(api) {
    ...
    return {
        config,
        complete: async ({ provider, model, apiKey, providerApi, authProfileId, agentDir, runtimeConfig, messages, system, maxTokens, temperature, reasoning, }) => {
            try {
                const piAiModuleId = "@mariozechner/pi-ai";
                const mod = (await import(piAiModuleId));
                ...
                const knownModel = typeof mod.getModel === "function" ? mod.getModel(providerId, modelId) : undefined;
                const fallbackApi = providerApi?.trim() ||
                ...
                const resolvedModel = isRecord(knownModel) &&
                ...
                if (!isRecord(result)) {
                    return {
                        ...
                        request_reasoning: typeof reasoning === "string" && reasoning.trim() ? reasoning.trim() : "(none)",
                        request_has_system: typeof system === "string" && system.trim().length > 0 ? "true" : "false",
                        request_temperature: typeof completeOptions.temperature === "number"
...
const lcmPlugin = {
    ...
    register(api) {
        ...
        api.logger.info(`[lcm] Plugin loaded (enabled=${deps.config.enabled}, db=${deps.config.databasePath}, backend=${deps.config.backend}, threshold=${deps.config.contextThreshold})`);
    },
};
