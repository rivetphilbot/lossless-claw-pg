...
async function testPostgres() {
    ...
    const tests = [
        ...
        test("Regex search messages", async () => {
            ...
            if (results.length < 1)
                throw new Error("No regex search results");
        }),
        ...
        test("Search summaries", async () => {
            ...
            if (results.length < 1)
                throw new Error("No summary search results");
        }),
        ...
        test("Get context items", async () => {
            ...
            if (items.length < 3)
                throw new Error(`Expected at least 3 context items, got ${items.length}`);
        }),
        test("Create large file", async () => {
            ...
            if (file.fileId !== "test-file-1")
                throw new Error("Large file creation failed");
        }),
        test("Get large files", async () => {
            ...
            if (files.length !== 1)
                throw new Error(`Expected 1 large file, got ${files.length}`);
        }),
    ];
...
//# sourceMappingURL=test-postgres.js.map