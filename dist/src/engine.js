...
import { createLcmConnection } from "./db/connection.js";
import { getLcmDbFeatures } from "./db/features.js";
import { runLcmMigrations } from "./db/migration.js";
import { createDelegatedExpansionGrant, removeDelegatedExpansionGrantForSession, revokeDelegatedExpansionGrantForSession, } from "./expansion-auth.js";
import { extensionFromNameOrMime, formatFileReference, generateExplorationSummary, parseFileBlocks, } from "./large-files.js";
...
function normalizeUnknownBlock(value) {
...
function toRuntimeRoleForTokenEstimate(role) {
...
function estimateMessageContentTokensForAfterTurn(content) {
    if (typeof content === "string") {
        return estimateTokens(content);
    }
    if (Array.isArray(content)) {
...
function estimateSessionTokenCountForAfterTurn(messages) {
    let total = 0;
    for (const message of messages) {
        ...
        if ("command" in message || "output" in message) {
            const commandText = typeof message.command === "string"
                ? message.command
                : "";
            const outputText = typeof message.output === "string"
                ? message.output
                : "";
...
function readLeafPathMessages(sessionFile) {
    let raw = "";
    try {
    ...
    for (const line of lines) {
        ...
        try {
            ...
            const candidate = parsed && typeof parsed === "object" && "message" in parsed
                ? parsed.message
                : parsed;
...
function messageIdentity(role, content) {
    return `${role}\u0000${content}`;
}
// ── LcmContextEngine ────────────────────────────────────────────────────────
export class LcmContextEngine {
    ...
    largeFileTextSummarizerResolved = false;
    largeFileTextSummarizer;
    deps;
    constructor(deps) {
        this.deps = deps;
        ...
        this.assembler = new ContextAssembler(this.conversationStore, this.summaryStore, this.config.timezone);
        ...
        this.compaction = new CompactionEngine(this.conversationStore, this.summaryStore, compactionConfig);
    ...
    ensureMigrated() {
        ...
        if ('getUnderlyingDatabase' in db) {
            const sqliteDb = db.getUnderlyingDatabase();
     ...
     */
    async withSessionQueue(sessionId, operation) {
    ...
    async resolveConversationIdForSessionKey(sessionKey) {
        ...
        try {
            ...
            const conversation = await this.conversationStore.getConversationBySessionId(runtimeSessionId);
     ...
     */
    async resolveLargeFileTextSummarizer() {
        ...
        try {
            ...
            this.largeFileTextSummarizer = async (prompt) => {
    ...
    async interceptLargeFiles(params) {
        ...
        for (const block of blocks) {
            ...
            rewrittenSegments.push(formatFileReference({
                fileId,
                fileName: block.fileName,
                mimeType: block.mimeType,
                byteSize,
    ...
    async reconcileSessionTail(params) {
        ...
        if (latestIdentity === messageIdentity(latestHistorical.role, latestHistorical.content)) {
            const dbOccurrences = await this.conversationStore.countMessagesByIdentity(conversationId, latestDbMessage.role, latestDbMessage.content);
        ...
        let anchorIndex = -1;
        const historicalIdentityTotals = new Map();
        for (const stored of storedHistoricalMessages) {
        ...
        const historicalIdentityCountsAfterIndex = new Map();
        const dbIdentityCounts = new Map();
        for (let index = storedHistoricalMessages.length - 1; index >= 0; index--) {
            ...
            const exists = await this.conversationStore.hasMessage(conversationId, stored.role, stored.content);
            ...
            if (dbCountForIdentity === undefined) {
                dbCountForIdentity = await this.conversationStore.countMessagesByIdentity(conversationId, stored.role, stored.content);
    ...
    async bootstrap(params) {
        this.ensureMigrated();
        const result = await this.withSessionQueue(params.sessionId, async () => this.conversationStore.withTransaction(async () => {
            ...
            if (existingCount === 0) {
                ...
                await this.summaryStore.appendContextMessages(conversationId, inserted.map((record) => record.messageId));
                await this.conversationStore.markConversationBootstrapped(conversationId);
                ...
                if (this.config.pruneHeartbeatOk) {
                    ...
                    if (pruned > 0) {
                        console.error(`[lcm] bootstrap: pruned ${pruned} HEARTBEAT_OK messages from conversation ${conversationId}`);
                    }
                }
                return {
                    bootstrapped: true,
        ...
        if (this.config.pruneHeartbeatOk && result.bootstrapped === false) {
            try {
                ...
                if (conversation) {
                    ...
                    if (pruned > 0) {
                        console.error(`[lcm] bootstrap: retroactively pruned ${pruned} HEARTBEAT_OK messages from conversation ${conversation.conversationId}`);
                    }
                }
            }
            catch (err) {
                console.error(`[lcm] bootstrap: heartbeat pruning failed:`, err instanceof Error ? err.message : err);
            }
        }
        return result;
    }
    ...
    async afterTurn(params) {
        ...
        const tokenBudget = typeof params.tokenBudget === "number" &&
            Number.isFinite(params.tokenBudget) &&
            params.tokenBudget > 0
    ...
    async evaluateLeafTrigger(sessionId) {
        this.ensureMigrated();
    ...
    async compact(params) {
        ...
        return this.withSessionQueue(params.sessionId, async () => {
            ...
            const targetTokens = params.compactionTarget === "threshold" ? decision.threshold : tokenBudget;
            const liveContextStillExceedsTarget = observedTokens !== undefined && observedTokens >= targetTokens;
            ...
            const useSweep = manualCompactionRequested || forceCompaction || params.compactionTarget === "threshold";
            if (useSweep) {
     ...
     */
    async pruneHeartbeatOkTurns(conversationId) {
        ...
        for (let i = 0; i < allMessages.length; i++) {
            ...
            const turnMessageIds = [msg.messageId];
            for (let j = i - 1; j >= 0; j--) {
 ...
 */
function isHeartbeatOkContent(content) {
