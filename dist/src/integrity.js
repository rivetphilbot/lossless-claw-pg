...
export class IntegrityChecker {
    ...
    async checkContextItemsContiguous(conversationId) {
    ...
    async checkContextItemsValidRefs(conversationId) {
        const items = await this.summaryStore.getContextItems(conversationId);
        const danglingRefs = [];
    ...
    async checkSummariesHaveLineage(conversationId) {
    ...
    async checkNoOrphanSummaries(conversationId) {
        const summaries = await this.summaryStore.getSummariesByConversation(conversationId);
        const contextItems = await this.summaryStore.getContextItems(conversationId);
        // Build set of summary IDs that appear in context_items
        const contextSummaryIds = new Set(contextItems
        ...
        const orphans = [];
        for (const summary of summaries) {
            if (!contextSummaryIds.has(summary.summaryId) &&
    ...
    async checkContextTokenConsistency(conversationId) {
        const contextItems = await this.summaryStore.getContextItems(conversationId);
    ...
    async checkMessageSeqContiguous(conversationId) {
    ...
    async checkNoDuplicateContextRefs(conversationId) {
        const items = await this.summaryStore.getContextItems(conversationId);
        const seenMessageIds = new Map();
        const seenSummaryIds = new Map();
        const duplicates = [];
        for (const item of items) {
            if (item.itemType === "message" && item.messageId != null) {
 ...
 */
export function repairPlan(report) {
    const suggestions = [];
    for (const check of report.checks) {
        ...
        switch (check.name) {
            case "conversation_exists":
                suggestions.push(`Create or restore conversation ${report.conversationId} in the conversations table`);
                break;
            ...
            case "context_items_valid_refs": {
                ...
                if (details?.danglingRefs) {
                    for (const ref of details.danglingRefs) {
                        suggestions.push(`Remove context item at ordinal ${ref.ordinal} referencing missing ${ref.itemType} ${ref.refId}`);
                    }
                }
                else {
            ...
            case "summaries_have_lineage": {
                ...
                if (details?.missingLineage) {
                    for (const entry of details.missingLineage) {
                        if (entry.kind === "leaf") {
                            suggestions.push(`Add missing lineage for leaf summary ${entry.summaryId} (link to source messages via summary_messages)`);
                        }
                        else {
                            suggestions.push(`Add missing lineage for condensed summary ${entry.summaryId} (link to parent summaries via summary_parents)`);
                        }
                    }
                }
                else {
            ...
            case "context_token_consistency":
                suggestions.push("Recompute context token count to reconcile mismatch between item-level sum and aggregate query");
                break;
            case "message_seq_contiguous":
                suggestions.push("Resequence message seq values to eliminate gaps (renumber starting from 0)");
                break;
            case "no_duplicate_context_refs": {
                ...
                if (details?.duplicates) {
                    for (const dup of details.duplicates) {
                        ...
                        suggestions.push(`Deduplicate ${dup.refType} ${dup.refId}: keep ordinal ${keepOrdinal}, remove ordinals ${removeOrdinals}`);
                    }
                }
                else {
                    suggestions.push("Remove duplicate message_id or summary_id references from context items");
                }
                break;
            }
            default:
 ...
 */
export async function collectMetrics(conversationId, conversationStore, summaryStore) {
