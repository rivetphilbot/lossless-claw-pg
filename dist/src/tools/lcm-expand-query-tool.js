...
import { clearDelegatedExpansionContext, evaluateExpansionRecursionGuard, recordExpansionDelegationTelemetry, resolveExpansionRequestId, resolveNextExpansionDepth, stampDelegatedExpansionContext, } from "./lcm-expansion-recursion-guard.js";
...
const LcmExpandQuerySchema = Type.Object({
    ...
    query: Type.Optional(Type.String({
        description: "Text query used to find summaries via grep before expansion. Required when summaryIds is not provided.",
    })),
    prompt: Type.String({
        description: "Question to answer using expanded context.",
    }),
    conversationId: Type.Optional(Type.Number({
        description: "Conversation ID to scope expansion to. If omitted, uses the current session conversation.",
    })),
...
export function createLcmExpandQueryTool(input) {
    return {
        ...
        description: "Answer a focused question using delegated LCM expansion. " +
        ...
        async execute(_toolCallId, params) {
            ...
            try {
                ...
                const summaryIds = normalizeSummaryIds(candidates
                ...
                if (status !== "ok") {
                    return jsonResult({
                        error: typeof wait?.error === "string" && wait.error.trim()
                            ? wait.error
                ...
                const reply = input.deps.readLatestAssistantReply(Array.isArray(replyPayload.messages) ? replyPayload.messages : []);
...
//# sourceMappingURL=lcm-expand-query-tool.js.map